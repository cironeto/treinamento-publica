...
public class AlunoService {
	...
	public static DefaultTableModel listaAlunoFrequenciaPresenca(List<Aluno> alunos) {
		...
		defaultTableModel.addColumn("Frequência (%)");

		for (int i = 0; i < alunos.size(); i++) {
			defaultTableModel.addRow(new Object[] { 
					alunos.get(i).getNome(),
					df.format((((double)alunos.get(i).getPresenças().size()) / (double)Dados.ContagemAulas) * 100)
			});
		}
	...
	public static DefaultTableModel listaAlunoChamada(List<Aluno> alunos) {
		...
		defaultTableModel.addColumn("Nome");
		defaultTableModel.addColumn("Presença");
	...
	public static DefaultTableModel listaAluno(List<Aluno> alunos) {
		...
		defaultTableModel.addColumn("Nome");
		defaultTableModel.addColumn("Presenças");

		for (int i = 0; i < alunos.size(); i++) {
			defaultTableModel
					.addRow(new Object[] { i + 1, alunos.get(i).getNome(), alunos.get(i).getPresenças().toString() });
		}
	...
	public static DefaultTableModel listaDadosAlunoSelecionado(Aluno aluno) {
		...
		defaultTableModel.addRow(new Object[] { aluno.getId(), aluno.getNome(), aluno.getPresenças().toString() });
